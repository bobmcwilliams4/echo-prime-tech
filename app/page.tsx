    ...
    <div className="min-h-screen transition-colors duration-600" style={{ backgroundColor: 'var(--ept-bg)' }}>
      ...
      <section className="relative pt-44 pb-32 px-6 mesh-bg overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          {/* Stats strip */}
          <div className="mt-24 grid grid-cols-2 md:grid-cols-5 gap-px rounded-2xl overflow-hidden glow-sm animate-fade-up animate-fade-up-delay-4" style={{ backgroundColor: 'var(--ept-border)' }}>
            {[
              { value: live.engines, label: 'AI Engines' },
              { value: live.doctrines, label: 'Doctrine Blocks' },
      ...
      {/* ─── Stack Transparency / Credibility ─── */}
      <section className="py-16 px-6 border-b" style={{ borderColor: 'var(--ept-border)' }}>
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            <div className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'var(--ept-accent)' }}>Full Stack Transparency</div>
            <h2 className="text-2xl md:text-3xl font-extrabold" style={{ color: 'var(--ept-text)' }}>See exactly how it&apos;s built</h2>
            <p className="mt-3 text-base max-w-xl mx-auto" style={{ color: 'var(--ept-text-secondary)' }}>No black boxes. Every layer of our architecture is documented, benchmarked, and open to inspection.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6 mb-10">
            {[
              { title: '144+ Cloudflare Workers', desc: 'Edge-deployed microservices across 300+ cities. Zero cold starts. Each Worker has structured logging, health endpoints, and auth middleware.', link: '/architecture', cta: 'View Architecture' },
              { title: 'Real Production Benchmarks', desc: 'Median API response under 50ms. Engine queries under 2 seconds. All numbers from live production — no synthetic tests.', link: '/benchmarks', cta: 'See Benchmarks' },
              { title: 'Verified Case Studies', desc: '80% faster title examination. 62% average cost reduction. Real results across oil & gas, tax, legal, cybersecurity, and more.', link: '/case-studies', cta: 'Read Case Studies' },
            ].map((card, i) => (
              <Link key={i} href={card.link} className="group p-6 rounded-2xl border transition-all hover:scale-[1.02]" style={{ backgroundColor: 'var(--ept-card-bg)', borderColor: 'var(--ept-card-border)' }}>
                <h3 className="text-lg font-bold mb-2" style={{ color: 'var(--ept-text)' }}>{card.title}</h3>
                <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--ept-text-muted)' }}>{card.desc}</p>
                <span className="text-xs font-semibold uppercase tracking-wider transition-colors group-hover:opacity-80" style={{ color: 'var(--ept-accent)' }}>{card.cta} &rarr;</span>
              </Link>
            ))}
          </div>
          <div className="flex flex-wrap justify-center gap-x-8 gap-y-3 text-xs font-medium" style={{ color: 'var(--ept-text-muted)' }}>
            <span>Cloudflare Workers</span>
            <span>&middot;</span>
            <span>D1 SQL Databases</span>
            <span>&middot;</span>
            <span>R2 Object Storage</span>
            <span>&middot;</span>
            <span>KV Edge Cache</span>
            <span>&middot;</span>
      ...
      <section id="contact" className="py-32 px-6 mesh-bg relative" ref={ctaSection.ref}>
        ...
        <div className={`max-w-3xl mx-auto text-center relative transition-all duration-700 ${ctaSection.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-6">
            ...
            <a href="mailto:bobbymcwilliams@echo-op.com" className="text-sm underline" style={{ color: 'var(--ept-accent)' }}>Enterprise inquiries</a>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t py-16 px-6 transition-colors" style={{ borderColor: 'var(--ept-border)' }}>
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-12">
            ...
            <div>
              ...
              <div className="flex flex-col gap-2.5">
                ...
                <a href="mailto:bobbymcwilliams@echo-op.com" className="text-xs hover:opacity-80 transition-opacity" style={{ color: 'var(--ept-text-muted)' }}>bobbymcwilliams@echo-op.com</a>
          ...
          <div className="border-t pt-6 text-center" style={{ borderColor: 'var(--ept-border)' }}>
            <p className="text-xs mb-1" style={{ color: 'var(--ept-text-muted)' }}>Built with autonomous AI in Midland, Texas.</p>
