    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Link href="/dashboard/business/bookings" className="btn-glow inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: 'var(--ept-accent)', color: '#fff' }}>
            <Plus className="w-4 h-4" /> Booking
          </Link>
          <Link href="/dashboard/business/invoices" className="btn-outline inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold">
            <Plus className="w-4 h-4" /> Invoice
          </Link>
          <Link href="/dashboard/business/quotes" className="btn-outline hidden md:inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold">
      ...
      <div className="glass-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/dashboard/business/bookings" className="text-sm font-medium" style={{ color: 'var(--ept-accent)' }}>
            View All &rarr;
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'New Booking', href: '/dashboard/business/bookings', icon: CalendarDays, color: 'var(--ept-accent)' },
          { label: 'New Invoice', href: '/dashboard/business/invoices', icon: FileText, color: 'var(--ept-info, #3b82f6)' },
          { label: 'New Quote', href: '/dashboard/business/quotes', icon: ClipboardList, color: 'var(--ept-purple, #a855f7)' },
          { label: 'Analytics', href: '/dashboard/business/analytics', icon: BarChart3, color: 'var(--ept-success)' },
        ].map(link => (
          <Link
            key={link.href}
            href={link.href}
